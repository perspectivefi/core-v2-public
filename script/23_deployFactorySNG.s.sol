...
contract FactorySNGScript is Script, SpectraConfigReader, SpectraConfigWriter {
    string private outputFilePath = "script/output/23_deployFactorySNG";
    string private outputFilePrefix = "23_deployFactorySNG";
    ...
    string private configFilePath = "script/constants/23_deployFactorySNG";
    string private configFilePrefix = "23_deployFactorySNG";
    ...
    function run() public {
        ...
        if (forTest) {
            ...
            address FactorySNGProxy = address(
                new AMTransparentUpgradeableProxy(
                    ...
                    abi.encodeWithSelector(
                        FactorySNG(address(0)).initialize.selector, accessManager, curveFactoryAddress
                    )
                )
            );
            ...
            IAccessManager(accessManager).setTargetFunctionRole(FactorySNGProxy, factory_selector, Roles.REGISTRY_ROLE);
            ...
            curveFactoryAddress = getAddressFromFile(configFilePath, configFilePrefix, ".curveFactoryAddress");
            address factorySNGInstance = address(new FactorySNG(registry, rateOracleRegistry));
            logAddress("factorySNGImplementation", factorySNGInstance);
            ...
            logAddress("factorySNGProxy", FactorySNGProxy);
            console.log("Factory SNG proxy deployed at", FactorySNGProxy);
            IRegistry(registry).setFactory(FactorySNGProxy);
            bytes32 adminSlot = vm.load(address(FactorySNGProxy), ERC1967Utils.ADMIN_SLOT);
            address proxyAdmin = address(uint160(uint256(adminSlot)));
            logAddress("factorySNGProxyAdmin", proxyAdmin);
            console.log("Factory Proxy Admin Address:", address(proxyAdmin));
            IAccessManager(accessManager).setTargetFunctionRole(proxyAdmin, selectors_proxy_admin, Roles.UPGRADE_ROLE);
            IAccessManager(accessManager).setTargetFunctionRole(FactorySNGProxy, factory_selector, Roles.REGISTRY_ROLE);
            console.log("Function setTargetFunctionRole Role set for ProxyAdmin");

            // To execute by the DAO
            // // Whitelisting the factory proxy in the AccessManager
            // IAccessManager(accessManager).grantRole(Roles.ADMIN_ROLE, FactorySNGProxy, 0);
            // IAccessManager(accessManager).grantRole(Roles.REGISTRY_ROLE, FactorySNGProxy, 0);
        }
        vm.stopBroadcast();
        if (!forTest) {
