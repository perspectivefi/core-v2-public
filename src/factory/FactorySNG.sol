...
contract FactorySNG is IFactorySNG, AccessManagedUpgradeable {
    ...
    // selectors
    bytes4 constant SET_INIT_PRICE_SELECTOR = IRateAdjustmentOracle(address(0)).setInitialPrice.selector;
    ...
    bytes4 constant SET_REWARDS_PROXY_SELECTOR = IPrincipalToken(address(0)).setRewardsProxy.selector;
    ...
    function deployAll(
        ...
        bytes memory _encodedData =
            abi.encodeWithSelector(IPrincipalToken(address(0)).initialize.selector, _ibt, _duration, accessManager);
        ...
        IAccessManager(accessManager)
            .setTargetFunctionRole(pt, getClaimRewardsProxySelectors(), Roles.REWARDS_HARVESTER_ROLE);

        IAccessManager(accessManager)
            .setTargetFunctionRole(pt, getSetRewardsProxySelectors(), Roles.REWARDS_PROXY_SETTER_ROLE);
        ...
        _encodedData = abi.encodeWithSelector(IRateAdjustmentOracle(address(0)).initialize.selector, accessManager);
        ...
        IAccessManager(accessManager)
            .setTargetFunctionRole(
                rateAdjustmentOracle, getSetRateOracleInitialPriceSelectors(), Roles.RATE_ADJUSTMENT_ORACLE_SETTER_ROLE
            );
        ...
        IRateAdjustmentOracle(rateAdjustmentOracle)
            .post_initialize(block.timestamp, IPrincipalToken(pt).maturity(), _curvePoolParams.initial_price, curvePool);

        if (_initialLiquidityInIBT != 0) {
            _addInitialLiquidity(curvePool, _initialLiquidityInIBT, _minPTShares, _curvePoolParams.initial_price);
        }
    }
    ...
    function _addInitialLiquidity(
        ...
        uint256 ibtsToTokenize = _initialLiquidityInIBT.mulDiv(ptBalance, ibtBalanceInPT + ptBalance);
        ...
        uint256 amount1 = IPrincipalToken(pt).depositIBT(ibtsToTokenize, address(this), msg.sender, _minPTShares);
     ...
     */
    function _deployCurvePool(address[] memory _coins, CurvePoolParams memory _p)
        internal
        returns (address curvePoolAddr)
    {
        ...
        curvePoolAddr = IStableSwapNGFactory(curveFactory)
            .deploy_plain_pool(
                "Spectra-PT/IBT",
                "SPT-PT/IBT",
                _coins,
