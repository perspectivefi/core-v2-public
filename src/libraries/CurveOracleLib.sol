...
library CurveOracleLib {
    ...
    function getPTToIBTRateSNG(address pool) public view returns (uint256) {
        ...
        if (maturity <= block.timestamp) {
            ...
            return pt.getIBTUnit().mulDiv(storedRates[1], storedRates[0])
                .mulDiv(IStableSwapNG(pool).price_oracle(0), CURVE_UNIT);
        }
    }
    ...
    function getLPTToIBTRateSNG(address pool) internal view returns (uint256) {
        ...
        if (maturity <= block.timestamp) {
            return pt.previewRedeemForIBT(balPT.mulDiv(CURVE_UNIT, supplyLPT)) + balIBT.mulDiv(CURVE_UNIT, supplyLPT);
        } else {
            uint256 ptToIBTRate = getPTToIBTRateSNG(pool);
            return ((balPT.mulDiv(ptToIBTRate, pt.getIBTUnit())) + balIBT).mulDiv(CURVE_UNIT, supplyLPT);
        }
    }
    ...
    function getBoundedPTPrice(address pool, uint256 impliedRate) internal view returns (uint256) {
        ...
        uint256 ptZCB =
            PTPricingLib.getPTPriceZCBModel(pt.previewRedeem(pt.getIBTUnit()), impliedRate, block.timestamp, maturity);
