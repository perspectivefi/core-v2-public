...
library PrincipalTokenUtil {
    ...
    function computeYield(
        ...
        if (_oldPTRate == _ptRate && _ibtRate > _oldIBTRate) {
            ...
            if (_oldPTRate > _ptRate) {
                ...
                uint256 actualNegativeYieldInAssetRay =
                    _convertToAssetsWithRate(userYTBalanceInRay, _oldPTRate - _ptRate, Math.Rounding.Floor);
                if (_ibtRate >= _oldIBTRate) {
                    // both negative and positive yield happened, more positive
                    yieldInAssetRay = actualNegativeYieldInAssetRay
                        + _convertToAssetsWithRate(ibtOfPTInRay, _ibtRate - _oldIBTRate, Math.Rounding.Floor);
                } else {
                    // either both negative and positive yield happened, more negative
                    // or only negative yield happened
                    uint256 expectedNegativeYieldInAssetRay =
                        _convertToAssetsWithRate(ibtOfPTInRay, (_oldIBTRate - _ibtRate), Math.Rounding.Ceil);
                    yieldInAssetRay = expectedNegativeYieldInAssetRay > actualNegativeYieldInAssetRay
                        ? 0
                ...
                newYieldInIBTRay = _convertToSharesWithRate(yieldInAssetRay, _ibtRate, Math.Rounding.Floor);
            } else {
    ...
    function _computeTokenizationFee(uint256 _amount, address _pt, address _registry) internal view returns (uint256) {
        return _amount.mulDiv(IRegistry(_registry).getTokenizationFee(), FEE_DIVISOR, Math.Rounding.Ceil)
            .mulDiv(
                FEE_DIVISOR - IRegistry(_registry).getFeeReduction(_pt, msg.sender), FEE_DIVISOR, Math.Rounding.Ceil
            );
    }
     ...
     */
    function _computeFlashloanFee(uint256 _amount, address _registry) internal view returns (uint256) {
        return _amount.mulDiv(IRegistry(_registry).getPTFlashLoanFee(), FEE_DIVISOR, Math.Rounding.Ceil);
    }
}
