...
library CurvePoolUtil {
     ...
     */
    function previewAddLiquidity(address _curvePool, uint256[2] memory _amounts)
        external
        view
        returns (uint256 minMintAmount)
    {
        (bool success, bytes memory responseData) =
            _curvePool.staticcall(abi.encodeCall(ICurvePool(address(0)).calc_token_amount, (_amounts)));
        if (!success) {
     ...
     * @return minMintAmount The amount of expected LP tokens received for depositing the liquidity in the pool
     */
    function previewAddLiquidityNG(address _curvePool, uint256[2] memory _amounts)
        external
        view
        returns (uint256 minMintAmount)
    {
        (bool success, bytes memory responseData) =
            _curvePool.staticcall(abi.encodeCall(ICurveNGPool(address(0)).calc_token_amount, (_amounts, true)));
        if (!success) {
            revert FailedToFetchExpectedLPTokenAmount();
        }
        minMintAmount = abi.decode(responseData, (uint256));
     ...
     * @return minMintAmount The amount of expected LP tokens received for depositing the liquidity in the pool
     */
    function previewAddLiquiditySNG(address _curvePool, uint256[] memory _amounts)
        external
        view
        returns (uint256 minMintAmount)
    {
        ...
        (bool success, bytes memory responseData) =
            _curvePool.staticcall(abi.encodeCall(IStableSwapNG(address(0)).calc_token_amount, (_amounts, true)));
        if (!success) {
            revert FailedToFetchExpectedLPTokenAmount();
        }
        minMintAmount = abi.decode(responseData, (uint256));
     ...
     */
    function previewRemoveLiquidity(address _curvePool, uint256 _lpTokenAmount)
        external
        view
        returns (uint256[2] memory minAmounts)
    {
        ...
        if (_lpTokenAmount > APPROXIMATION_DECREMENT && totalSupply != 0) {
            ...
            minAmounts = [(ibtBalance * _lpTokenAmount) / totalSupply, (ptBalance * _lpTokenAmount) / totalSupply];
        } else {
     ...
     * @return minAmounts The expected respective amounts of IBT and PT withdrawn from the curve pool
     */
    function previewRemoveLiquidityNG(address _curvePool, uint256 _lpTokenAmount)
        external
        view
        returns (uint256[2] memory minAmounts)
    {
        ...
        if (_lpTokenAmount == totalSupply) {
            ...
            _lpTokenAmount -= APPROXIMATION_DECREMENT;
            minAmounts = [ibtBalance.mulDiv(_lpTokenAmount, totalSupply), ptBalance.mulDiv(_lpTokenAmount, totalSupply)];
        } else {
            minAmounts = [uint256(0), uint256(0)];
        }
    }
     ...
     * @return minAmounts The expected respective amounts of IBT and PT withdrawn from the curve pool
     */
    function previewRemoveLiquiditySNG(address _curvePool, uint256 _lpTokenAmount)
        external
        view
     ...
     */
    function previewRemoveLiquidityOneCoin(address _curvePool, uint256 _lpTokenAmount, uint256 _i)
        external
        view
     ...
     * @return minAmount The expected amount of coin i withdrawn from the curve pool
     */
    function previewRemoveLiquidityOneCoinNG(address _curvePool, uint256 _lpTokenAmount, uint256 _i)
        external
        view
        returns (uint256 minAmount)
    {
     ...
     * @return minAmount The expected amount of coin i withdrawn from the curve pool
     */
    function previewRemoveLiquidityOneCoinSNG(address _curvePool, uint256 _lpTokenAmount, int128 _i)
        external
        view
        returns (uint256 minAmount)
    {
     ...
     */
    function calcIBTsToTokenizeForCurvePool(uint256 _amount, address _curvePool, address _pt)
        external
        view
     ...
     * @return ibts The amount of IBT which will be deposited in the curve pool
     */
    function calcIBTsToTokenizeForCurvePoolCustomProp(uint256 _amount, uint256 _prop, address _pt)
        external
        view
        returns (uint256 ibts)
    {
     ...
     */
    function getDx(address _curvePool, uint256 _i, uint256 _j, uint256 _targetDy) external view returns (uint256 dx) {
        // Initial guesses
        ...
        while (!_dxSolved(_curvePool, _i, _j, _guess, _targetDy, _minGuess, _maxGuess)) {
            ...
            (_minGuess, _maxGuess, _guess) =
                _runLoop(_minGuess, _maxGuess, _factor100, _guess, _targetDy, _curvePool, _i, _j);
