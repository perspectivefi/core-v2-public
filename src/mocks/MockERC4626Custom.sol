...
abstract contract MockERC4626Custom is ERC4626 {
     ...
     */
    constructor(string memory name_, string memory symbol_, IERC20 asset_, uint8 decimals_, uint16 rateChange_)
        ERC20(name_, symbol_)
        ERC4626(asset_)
    {
        _decimals = decimals_;
     ...
     */
    function depositRateIncreasesConstant(uint256 assets, address receiver) public virtual returns (uint256) {
     ...
     */
    function depositFeeThreshold(uint256 threshold, uint256 lowFee, uint256 highFee, uint256 assets, address receiver)
        public
        virtual
        returns (uint256)
    {
     ...
     */
    function previewDepositFeeThreshold(uint256 threshold, uint256 lowFee, uint256 highFee, uint256 assets)
        public
        view
        virtual
        returns (uint256)
    {
        uint256 _assets = assets <= threshold ? assets.mulDiv(unit - lowFee, unit) : assets.mulDiv(unit - highFee, unit);
     ...
     */
    function previewRedeemFeeThreshold(uint256 threshold, uint256 lowFee, uint256 highFee, uint256 shares)
        public
        view
        virtual
        returns (uint256)
     ...
     */
    function depositRateIncreasesProportionally(uint256 assets, address receiver) public virtual returns (uint256) {
        uint256 maxAssets = maxDeposit(receiver);
        if (assets > maxAssets) {
            revert ERC4626ExceededMaxDeposit(receiver, assets, maxAssets);
        }
     ...
     */
    function depositRateDecreasesConstant(uint256 assets, address receiver) public virtual returns (uint256) {
        uint256 maxAssets = maxDeposit(receiver);
        if (assets > maxAssets) {
            revert ERC4626ExceededMaxDeposit(receiver, assets, maxAssets);
        }
     ...
     */
    function depositRateDecreasesProportionally(uint256 assets, address receiver) public virtual returns (uint256) {
        uint256 maxAssets = maxDeposit(receiver);
        if (assets > maxAssets) {
            revert ERC4626ExceededMaxDeposit(receiver, assets, maxAssets);
        }
