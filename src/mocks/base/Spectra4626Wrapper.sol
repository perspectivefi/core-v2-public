...
abstract contract Spectra4626Wrapper is ERC4626Upgradeable, AccessManagedUpgradeable, ISpectra4626Wrapper {
    ...
    function _getSpectra4626WrapperStorage() private pure returns (Spectra4626WrapperStorage storage $) {
        assembly {
    ...
    function __Spectra4626Wrapper_init(address asset_, address vaultShare_, address initialAuthority_)
        internal
        onlyInitializing
    {
    ...
    function decimals() public view virtual override(IERC20Metadata, ERC4626Upgradeable) returns (uint8) {
    ...
    /// @dev See {ISpectra4626Wrapper-wrap}.
    function wrap(uint256 vaultShares, address receiver, uint256 minShares) public virtual returns (uint256) {
    ...
    function unwrap(uint256 shares, address receiver, address owner) public virtual returns (uint256) {
        address caller = _msgSender();
        if (caller != owner) {
    ...
    /// @dev See {ISpectra4626Wrapper-unwrap}.
    function unwrap(uint256 shares, address receiver, address owner, uint256 minVaultShares)
        public
        virtual
        returns (uint256)
    {
    ...
    function _previewWrap(uint256 vaultShares, Math.Rounding rounding) internal view virtual returns (uint256) {
        return vaultShares.mulDiv(totalSupply() + 10 ** _decimalsOffset(), totalVaultShares() + 1, rounding);
    }

    /// @dev Internal conversion function (from wrapper shares to vault shares) with support for rounding direction.
    function _previewUnwrap(uint256 shares, Math.Rounding rounding) internal view virtual returns (uint256) {
        return shares.mulDiv(totalVaultShares() + 1, totalSupply() + 10 ** _decimalsOffset(), rounding);
    }

    /// @dev Internal getter to build wrapper name
    function _wrapperName() internal view virtual returns (string memory wrapperName) {
        wrapperName = string.concat("Spectra ERC4626 Wrapper: ", IERC20Metadata(vaultShare()).name());
    }
