    ...
    IPrincipalToken
{
    ...
    event FeeClaimed(address indexed user, uint256 indexed redeemedIbts);
     ...
     */
    function initialize(address _ibt, uint256 _duration, address _initialAuthority) external initializer {
        ...
        string memory _underlyingSymbol = IERC4626(IERC4626(_ibt).asset()).symbol();
        string memory _name = string.concat(NamingUtil.genPTName(_ibtSymbol, _underlyingSymbol, _expiry), "V2");
        __ERC20_init(_name, string.concat(NamingUtil.genPTSymbol(_ibtSymbol, _underlyingSymbol, _expiry), "V2"));
        ...
        if (_underlyingDecimals < MIN_DECIMALS || _underlyingDecimals > _ibtDecimals || _ibtDecimals > MAX_DECIMALS) {
        ...
        yt = _deployYT(
            NamingUtil.genYTName(_ibtSymbol, _underlyingSymbol, _expiry),
            NamingUtil.genYTSymbol(_ibtSymbol, _underlyingSymbol, _expiry)
        );
    }
     ...
     * @dev See {IPrincipalToken-deposit}.
     */
    function deposit(uint256 assets, address ptReceiver, address ytReceiver)
        public
        override
        nonReentrant
     ...
     * @dev See {IPrincipalToken-depositIBT}.
     */
    function depositIBT(uint256 ibts, address ptReceiver, address ytReceiver)
        public
        override
        nonReentrant
        returns (uint256 shares)
     ...
     */
    function claimFees() external override whenNotPaused returns (uint256 ibts) {
        if (msg.sender != IRegistry(registry).getFeeCollector()) {
            revert UnauthorizedCaller();
        }
        ibts = unclaimedFeesInIBT;
        unclaimedFeesInIBT = 0;
        emit FeeClaimed(msg.sender, ibts);
        IERC20(ibt).safeTransfer(msg.sender, ibts);
    }

    /**
     * @dev See {IPrincipalToken-updateYield}.
     */
    function updateYield(address _user) public override whenNotPaused returns (uint256 updatedUserYieldInIBT) {
        ...
        if (_oldIBTRateUser != 0) {
            updatedUserYieldInIBT =
                _user.computeYield(yieldOfUserInIBT[_user], _oldIBTRateUser, _ibtRate, _oldPTRateUser, _ptRate, yt);
     ...
     */
    function claimYield(address _receiver, uint256 _minAssets) public override returns (uint256 yieldInAsset) {
     ...
     */
    function claimYieldInIBT(address _receiver, uint256 _minIBT) public override returns (uint256 yieldInIBT) {
     ...
     */
    function previewDepositIBT(uint256 ibts) public view override notExpired whenNotPaused returns (uint256) {
     ...
     */
    function previewRedeemForIBT(uint256 shares) public view override whenNotPaused returns (uint256) {
     ...
     */
    function getCurrentYieldOfUserInIBT(address _user) external view override returns (uint256 _yieldOfUserInIBT) {
        uint256 _oldIBTRate = ibtRateOfUser[_user];
        if (_oldIBTRate != 0) {
            (uint256 _ptRate, uint256 _ibtRate) = _getPTandIBTRates(false);
            uint256 _oldPTRate = ptRateOfUser[_user];
            _yieldOfUserInIBT =
                _user.computeYield(yieldOfUserInIBT[_user], _oldIBTRate, _ibtRate, _oldPTRate, _ptRate, yt);
     ...
     */
    function _convertSharesToIBTs(uint256 _shares, bool _roundUp) internal view returns (uint256 ibts) {
        (uint256 _ptRate, uint256 _ibtRate) = _getPTandIBTRates(false);
        if (_ibtRate == 0) {
            revert RateError();
        }
        ibts = _shares.mulDiv(_ptRate, _ibtRate, _roundUp ? Math.Rounding.Ceil : Math.Rounding.Floor);
    }
     ...
     */
    function _convertIBTsToShares(uint256 _ibts, bool _roundUp) internal view returns (uint256 shares) {
        (uint256 _ptRate, uint256 _ibtRate) = _getPTandIBTRates(false);
        if (_ptRate == 0) {
            revert RateError();
        }
        shares = _ibts.mulDiv(_ibtRate, _ptRate, _roundUp ? Math.Rounding.Ceil : Math.Rounding.Floor);
    }
    ...
    function _deployYT(string memory _name, string memory _symbol) internal returns (address _yt) {
        ...
        _yt = address(
            new BeaconProxy(
                ytBeacon, abi.encodeCall(IYieldToken(address(0)).initialize, (_name, _symbol, address(this)))
            )
        );
        emit YTDeployed(_yt);
    }
     ...
     */
    function _depositIBT(uint256 _ibts, address _ptReceiver, address _ytReceiver)
        internal
        notExpired
        returns (uint256 shares)
    {
     ...
     */
    function _burnSharesForWithdraw(uint256 _ibts, address _receiver, address _owner, uint256 _ptRate, uint256 _ibtRate)
        internal
        returns (uint256 shares)
    {
        if (_ptRate == 0) {
     ...
     * @return _ibtRate The new IBT rate
     */
    function _getCurrentPTandIBTRates(bool roundUpPTRate) internal view returns (uint256 _ptRate, uint256 _ibtRate) {
        address _ibt = ibt;
        ...
        _ptRate = _ibtRate < ibtRate
            ? ptRate.mulDiv(_ibtRate, ibtRate, roundUpPTRate ? Math.Rounding.Ceil : Math.Rounding.Floor)
            : ptRate;
    }
     ...
     * @return _ibtRate The new IBT rate
     */
    function _getPTandIBTRates(bool roundUpPTRate) internal view returns (uint256 _ptRate, uint256 _ibtRate) {
        if (ratesAtExpiryStored == RAE_NOT_STORED) {
