...
contract MockRewardsProxy is IRewardsProxy {
    ...
    function claimRewards(bytes memory data) external override {
        (address rewardsContract, uint256 amountToClaim, address rewardsReceiver, address tokenAddress) =
            abi.decode(data, (address, uint256, address, address));
        (bool success,) = rewardsContract.call(abi.encodeWithSignature("claim(uint256)", amountToClaim));
