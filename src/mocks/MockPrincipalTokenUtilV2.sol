...
library MockPrincipalTokenUtilV2 {
    ...
    function computeYield(
        ...
        external
        pure
        returns (uint256 updatedYieldInIBT)
    {
    ...
    function tryGetTokenDecimals(address _token) external view returns (uint8) {
        (bool success, bytes memory encodedDecimals) = _token.staticcall(abi.encodeCall(IERC20Metadata.decimals, ()));
     ...
     */
    function _convertToSharesWithRate(uint256 _assetsInRay, uint256 _rate, Math.Rounding _rounding)
        internal
        pure
        returns (uint256 sharesInRay)
    {
        if (_rate == 0) {
     ...
     */
    function _convertToAssetsWithRate(uint256 _sharesInRay, uint256 _rate, Math.Rounding _rounding)
        internal
        pure
        returns (uint256 assetsInRay)
    {
