    ...
    IPrincipalToken
{
    ...
    function initialize(address _ibt, uint256 _duration, address _initialAuthority) external initializer {
        ...
        string memory _underlyingSymbol = IERC4626(underlying_).symbol();
        string memory _name = NamingUtil.genPTName(_ibtSymbol, _underlyingSymbol, expiry);
        __ERC20_init(_name, NamingUtil.genPTSymbol(_ibtSymbol, _underlyingSymbol, expiry));
        ...
        yt = _deployYT(
            NamingUtil.genYTName(_ibtSymbol, _underlyingSymbol, expiry),
            NamingUtil.genYTSymbol(_ibtSymbol, _underlyingSymbol, expiry)
        );
    }
